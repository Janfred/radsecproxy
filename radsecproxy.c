...
int radsrv(struct request *rq) {
    ...
    if ((to->conf->loopprevention == 1 || (to->conf->loopprevention == UCHAR_MAX && options.loopprevention == 1)) && !strcmp(from->conf->name, to->conf->name)) {
...
void replyh(struct server *server, uint8_t *buf, int len) {
    ...
    if (msg->code != RAD_Access_Accept && msg->code != RAD_Access_Reject && msg->code != RAD_Access_Challenge && msg->code != RAD_Accounting_Response) {
...
int confclient_cb(struct gconffile **cf, void *arg, char *block, char *opt, char *val) {
    ...
    if (conf->type == RAD_TLS || conf->type == RAD_DTLS) {
        conf->tlsconf = conf->tls ? tlsgettls(conf->tls, NULL) : conf->pskkey ? tlsgetdefaultpsk()
...
void getmainconfig(const char *configfile) {
    ...
    if (log_mac_str != NULL) {
        ...
        } else if ((options.log_mac == RSP_MAC_VENDOR_KEY_HASHED || options.log_mac == RSP_MAC_FULLY_KEY_HASHED)) {
