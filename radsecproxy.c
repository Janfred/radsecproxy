...
void write_debug_to_socket(struct client *client, struct server *server, uint8_t *buf, uint8_t request){
    struct clsrvconf *from, *to;
    uint32_t from_sock, to_sock;
    uint16_t len, from_length, to_length, htons_from_length, htons_to_length;
    ...
    if(radius_cap.active){
        ...
        from_length = strlen(from->name);
        to_length = strlen(to->name);
        htons_from_length = htons(from_length);
        htons_to_length = htons(to_length);

        debug(DBG_INFO, "Writing data to socket");

        total_length = 13+from_length+to_length+len;

        towrite = malloc(total_length);
        ind = 0;
