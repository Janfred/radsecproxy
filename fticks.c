...
int fticks_configure(struct options *options,
    ...
    if (*keyp != NULL) {
        ...
        if (options->fticks_mac != RSP_MAC_VENDOR_KEY_HASHED && options->fticks_mac != RSP_MAC_FULLY_KEY_HASHED)
            debugx(1, DBG_WARN, "config warning: FTicksKey not used");
    } else if (options->fticks_reporting != RSP_FTICKS_REPORTING_NONE && (options->fticks_mac == RSP_MAC_VENDOR_KEY_HASHED || options->fticks_mac == RSP_MAC_FULLY_KEY_HASHED)) {
        debugx(1, DBG_ERR,
